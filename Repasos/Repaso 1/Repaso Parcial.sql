...
ON P.DNI = T.DNI
WHERE strftime('%Y', T.fecha) = '2021' -- Pacientes atendidos en 2021
AND P.DNI NOT IN (
    SELECT T2.DNI
    FROM Turno T2
    WHERE strftime('%Y', T2.fecha) = '2019' -- Pacientes atendidos en 2019
);
